<?php
	...
	{
		$id=intval($_POST['cid']);
		$stmt = $DB_con->prepare("SELECT * FROM contenido WHERE cshort = :id");
	$stmt->execute(array(':id' => $id));
	
	while($row=$stmt->fetch(PDO::FETCH_ASSOC))
	{
	
	echo strtoupper($row['sub1']."+".$row['sub2']."+ ".$row['sub3']."+ ".$row['sub4']."+ ".$row['sub5']."+ ".$row['sub6']."+ ".$row['sub7']); 
	
	}
	
	}
	//aquí esta recibiendo la id del usuario?
	?>	